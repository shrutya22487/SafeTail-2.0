...
Small library to load a saved predict regressor (pickle) and predict on a chosen row
from a CSV that matches the benchmark schema. Assumes 0-based row indexing.

Usage example (from another file):
    from predict_predictor import PredictPredictor
    pp = PredictPredictor(verbose=True)
    out = pp.predict_row(row_index=0)
...
class PredictPredictor:
    def __init__(self, verbose: bool = False):
        """
        Instantiate and load the predict regressor pickle.

        Args:
            verbose: if True, prints extra information during load/predict
        """
        base_dir = Path(__file__).resolve().parent
        # project_root/models/server1/predict_regressor_model.pkl
        self.model_path: Path = base_dir.parent.parent / "models" / "server1" / "predict_regressor_model.pkl"
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            if isinstance(data, Pipeline):
                ...
                if self.verbose:
                    print(f"[PredictPredictor] Loaded sklearn Pipeline from {self.model_path}")
                return
        except Exception:
            pass
        ...
        if isinstance(data, dict):
            ...
            if self.verbose:
                print(f"[PredictPredictor] Loaded dict pickle. Model: {self.model_name}")
            if self.model is None:
                raise ModelLoadError(f"Pickle at {self.model_path} is a dict but contains no 'model' key.")
            return

        # assume data is the model
        self.model = data
        self.model_name = type(self.model).__name__
        if self.verbose:
            print(f"[PredictPredictor] Loaded model object of type {self.model_name}")
    ...
    def _prepare_input(self, df: pd.DataFrame, row_index: int, feature_columns: Optional[List[str]] = None) -> Tuple[np.ndarray, List[str]]:
        ...
        if missing:
            ...
            if self.verbose:
                print(f"[PredictPredictor] Warning: missing feature columns in CSV: {missing}. Falling back to inferred: {inferred}")
    ...
    def predict_row(self, row_index: int, feature_columns: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        Predict for the given 0-based row_index in the CSV.

        Returns dict with prediction, model_name, feature_columns and feature_values.
        """
        ...
        if self.verbose:
            print(f"[PredictPredictor] CSV loaded: {self.csv_path} ({len(df)} rows)")
        ...
        if not is_pipeline and self.scaler is not None:
            ...
            if any(tok in clsname for tok in ('linear', 'ridge', 'lasso', 'elastic', 'sgd', 'orthogonal')):
                try:
                    ...
                    if self.verbose:
                        print("[PredictPredictor] Applied saved scaler to input features.")
                except Exception as e:
                    if self.verbose:
                        print("[PredictPredictor] Failed to apply scaler; using raw features. Error:", e)
                    X_for_pred = X_raw
            else:
                if self.verbose:
                    print("[PredictPredictor] Scaler present but not applied (model looks tree-based).")
                X_for_pred = X_raw
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Quick CLI for PredictPredictor (0-based indexing).")
    parser.add_argument("row_index", type=int, help="0-based row index to predict")
    parser.add_argument("--verbose", action="store_true", help="Verbose")
    args = parser.parse_args()

    pp = PredictPredictor(verbose=args.verbose)
    out = pp.predict_row(row_index=args.row_index)
    print(out)
